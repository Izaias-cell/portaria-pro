...
export function UnitPhoneManager({ unitPhones, onUpdate }: UnitPhoneManagerProps) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Vínculos WhatsApp</h2>
          <p className="text-[10px] font-bold text-slate-400 uppercase">Gerencie os telefones autorizados por unidade</p>
        </div>
        <button
          onClick={handleAdd}
      ...
      <div className="grid gap-3">
        {filteredPhones.length === 0 ? (
          <div className="p-12 text-center text-slate-400 bg-white rounded-3xl border border-dashed border-slate-200">
            ...
            <p className="text-sm font-bold">Nenhum telefone vinculado encontrado.</p>
          </div>
        ) : (
          filteredPhones.map((p) => (
            <div
              key={p.id}
              className={cn(
                "bg-white border rounded-2xl p-4 shadow-sm flex items-center gap-4 transition-all hover:shadow-md",
